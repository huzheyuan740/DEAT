...
class UPDeT(nn.Module):
    ...
    def forward(self, inputs, hidden_state, task_enemy_num, task_ally_num):
        # print("self.hidden_states:", hidden_state.shape)
        outputs, _ = self.transformer.forward(inputs, hidden_state, None)
        # first output for 6 action (no_op stop up down left right)
        # print("outputs:", outputs.shape)

        q_basic_actions = self.q_basic(outputs[:, :-1, :])
        # for i in range(task_ally_num - 1):
        #     # print("index:", i+1)
        #     q_basic_actions_each = self.q_basic(outputs[:, i+1, :])
        #     q_basic_actions = torch.cat((q_basic_actions, q_basic_actions_each), 0)
        # print("q_basic_actions:", q_basic_actions.shape)
